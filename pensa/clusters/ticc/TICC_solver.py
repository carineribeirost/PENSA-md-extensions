...
from pensa.clusters.ticc.src.TICC_helper import *
...
class TICC:
    ...
    def fit_array(self, data):
        """
        Main method for TICC solver — accepts a numpy array directly.

        Parameters
        ----------
        data : np.ndarray, shape (n_frames, n_features)
            Time-series data as a 2-D float array.

        Returns
        -------
        Same as fit(): (clustered_points, train_cluster_inverse)
        or (clustered_points, train_cluster_inverse, bic) if compute_BIC=True.
        """
        assert self.maxIters > 0
        self.log_parameters()

        times_series_arr = np.array(data, dtype=float)
                    ...
                    if len_new_train_clusters[cluster_num] == 0:
                        ...
                        for i in range(0, self.cluster_reassignment):
                            ...
                            if point_to_move >= len(clustered_points):
                                break
                            clustered_points[point_to_move] = cluster_num
                            computed_covariance[self.number_of_clusters, cluster_num] = \
                                old_computed_covariance[self.number_of_clusters, cluster_selected]
        ...
        if self.compute_BIC:
            bic = computeBIC(self.number_of_clusters, time_series_rows_size, clustered_points,
